...
from ctzzy.utils.string import string_without_prefix
from ctzzy.utils.encoding import sha256_digest
...
def is_ev_cert(ee_cert):
    ...
    if all_extensions is not None:
        ...
        if len(policy_extensions) > 0:
            ...
            sequence_der = policy_extension['extnValue']
            try:
        ...
        for rdn in issuer['rdnSequence']:
            for item in rdn:
                if item.getComponentByName('type') == organization_name_oid:
                    ...
                    organization_name = string_without_prefix('\x13\r',
                                                              organisation_name)
                    if organization_name == "Let's Encrypt":
                        return True
    return False
...
TbsCert = namedtuple(
    ...
    lazy_vals={
        ...
        # cf. https://tools.ietf.org/html/rfc6962#section-3.2      <1..2^24-1> 看不懂
